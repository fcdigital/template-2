...
const fonter = require('gulp-fonter-fix'); // плагин конвертирующий otf, ttf в woff, woff2
...
gulp.task('otfToTtf', () => {
	...
	return (
		gulp
			...
			.pipe(
				fonter({
					formats: ['ttf'], // плагин fonter конвертирует форматы
				})
			)
			// Выгружаем в исходную папку
			.pipe(gulp.dest(`${srcFolder}/fonts/`))
			.pipe(
				plumber(  // плагин plumber предотвращает ошибки при обработке галпом
					notify.onError({
						title: 'FONTS',
						message:
...
gulp.task('fontsStyle', () => {
	...
	fs.readdir(`${destFolder}/fonts/`, function (err, fontsFiles) { // проверяет есть ли он в build (в src надо добавлять самому)
		if (fontsFiles) {                                                   
